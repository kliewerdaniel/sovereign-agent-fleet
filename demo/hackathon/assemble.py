...
CLIPS = HERE / "clips"
AUDIO = HERE / "audio"
CAP_TXT = HERE / "caption_concat.txt"
OUT = HERE / "sovereign_agent_fleet_hackathon.mp4"

# 1) visual concat of per-beat clips (each already exactly dur long)
...
# 3) overlay caption track + mux narration (NEVER -r on the caption INPUT)
subprocess.run([FF, "-y", "-hide_banner", "-i", str(vis),
                "-f", "concat", "-safe", "0", "-i", str(CAP_TXT),
                "-i", str(NARR),
                "-filter_complex", "[1:v]format=rgba[cap];[0:v][cap]overlay=0:0[vout]",
                "-map", "[vout]", "-map", "2:a",
