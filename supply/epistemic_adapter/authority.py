"""Phase 4 (M0) — governance issuer side of the THIRD adapter.

Structurally identical to ``exchange/epistemic_adapter/authority.py`` and
``incident/epistemic_adapter/authority.py``. The neutral substrate
(``fleet.epistemic``) deliberately CANNOT mint authority; ``AuthorityGrant`` is a
data holder and ``verify_grant`` only validates. This adapter holds the trusted
issuer Ed25519 key for the supply domain and signs grants that ``decide()`` later
verifies against ``public_key_pem``.

A THIRD, identical issuer drives home the point: the substrate cannot distinguish
...
class GovernanceAuthority:
    """The trusted issuer of AuthorityGrants for the supply domain.
    ...
    governance_role: str = "SupplyChainOps"
