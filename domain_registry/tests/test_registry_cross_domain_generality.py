...
def test_registry_table_has_all_four_consumers():
    """The registry enumerates exactly the six external consumers; capability
    ...
    assert labels == [
        ...
        "mirror/self-observability", "grid/energy",
    ]
    assert len(set(caps)) == 6 == len(caps)
...
def test_m0_same_policy_same_verdict_across_all_registered_domains(gov):
    ...
    assert reasons == ["granted", "granted", "granted", "granted", "granted", "granted"]
    # Distinct capability literals survive through to the decision...
    assert len({r.capability for r in results}) == 6
...
def test_m0_policy_flip_changes_all_registered_domains_identically(gov):
    ...
    assert [r.verdict for r in auto] == ["AUTO", "AUTO", "AUTO", "AUTO", "AUTO", "AUTO"]
    assert [r.verdict for r in human] == ["HUMAN", "HUMAN", "HUMAN", "HUMAN", "HUMAN", "HUMAN"]
...
def test_m0_no_shared_substrate_state_among_registered_domains(gov):
    ...
    assert verdicts == ["AUTO", "AUTO", "AUTO", "AUTO", "AUTO", "AUTO", "AUTO"]
...
def test_reverse_epistemic_layer_does_not_import_any_registered_adapter():
    ...
    bad = (
        ...
        "grid.epistemic_adapter", "grid.sim",
