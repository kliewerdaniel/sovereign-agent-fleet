"""Phase 5 (M0) — governance issuer side of the FOURTH adapter.

Structurally identical to ``exchange/epistemic_adapter/authority.py``,
``incident/epistemic_adapter/authority.py``, and ``supply/epistemic_adapter/
authority.py``. The neutral substrate (``fleet.epistemic``) deliberately CANNOT
mint authority; ``AuthorityGrant`` is a data holder and ``verify_grant`` only
validates. This adapter holds the trusted issuer Ed25519 key for the hypothesis
domain and signs grants that ``decide()`` later verifies against
``public_key_pem``.

A FOURTH, identical issuer drives home the point: the substrate cannot distinguish
which domain's grant it is verifying. Its decision reads only the grant's
scope/epoch and the deterministic policy — never the domain.
"""
...
class GovernanceAuthority:
    """The trusted issuer of AuthorityGrants for the hypothesis domain.
    ...
    governance_role: str = "ResearchOps"
