...
# ===========================================================================
# C. CROSS-DOMAIN GENERALITY (M0 decisive proof)
# ===========================================================================
...
def test_m0_same_policy_same_verdict_across_domains():
    """The substrate returns the IDENTICAL verdict for the incident domain and the
    exchange domain when both present the same (valid grant, matching scope,
    AUTO policy). The semantic domain is irrelevant — only the literal capability
    string + policy matter."""
    ...
    assert inc.verdict == exc.verdict == "AUTO"
    # Same shape of decision object, same reason — no domain leakage.
    assert inc.reason == exc.reason == "granted"
    assert inc.capability != exc.capability  # different literal strings...
    assert inc.verdict == exc.verdict        # ...but identical substrate verdict


def test_m0_policy_flip_changes_both_domains_identically():
    """When policy flips (require_human_approval), BOTH domains move AUTO->HUMAN
    together. The substrate's behavior is a pure function of (grant, scope,
    policy) — never the domain. This is why 'substrate does not derive identity
    from first consumer' holds."""
    ...
    assert inc_auto.verdict == exc_auto.verdict == "AUTO"
    assert inc_human.verdict == exc_human.verdict == "HUMAN"
    # The flip is caused by policy, not domain: each domain's AUTO and HUMAN match.
    assert inc_auto.verdict == exc_auto.verdict
    assert inc_human.verdict == exc_human.verdict


def test_m0_no_shared_substrate_state_between_domains():
    """The substrate keeps no per-domain state. Two consecutive decisions with
    different domains use the same pure function and produce domain-independent
    results. (Guards against any accidental module-level domain cache.)"""
    results = []
    for cap in (iadapter.CAP_INCIDENT_REMEDIATE, eadapter.CAP_TRADE_EXECUTE,
                iadapter.CAP_INCIDENT_REMEDIATE):
        results.append(_neutral_decision(cap, True, False).verdict)
    assert results == ["AUTO", "AUTO", "AUTO"]
